...
| **Least-Privilege Credentials** | Master API keys never enter the sandbox or the database. A credential proxy stores keys in-memory; the sandbox only gets a per-run token. |
| **Row-Level Security** | The runner connects via a restricted Postgres role with RLS enforcing per-run data isolation. |
...
│   └── 002_runner_rls.sql      # Runner role + RLS policies
...
│   ├── Dockerfile.controlplane # Control plane image
│   ├── Dockerfile.credential-proxy  # Credential proxy image
│   ├── credential_proxy.py     # Credential proxy implementation
│   ├── tinyproxy.conf          # Tinyproxy configuration
│   ├── tinyproxy-allowlist.txt # Egress domain allowlist
│   └── entrypoint.sh           # Control plane entrypoint
└── tests/
    ├── unit/                   # Unit tests
